...
\node[det]  (a1)     at (3.1,0)  {\textbf{A1}\\[-1pt] Ingestão dos\\ documentos};
...
O Corpus definido na fase de desenvolvimenteo deste artefato foram todos os artigos completos e publicados nos \textit{Anais do Anais do XXII Simpósio Brasileiro de Sistemas de Informação}, coletados através do SBC OpenLib \cite{solsbc} e organizados em pastas, totalizando 68 artigos, definindo assim a trilha-alvo.

Os artigos da trilha-alvo, no formato PDF, são convertidos com Docling \cite{Docling}, preservando-se toda a sua estrutura original são convertidos para Markdown e Json. Esse objeto fornece segmentação de seções, tipagem de itens e contagem de tabelas e figuras sem recurso a expressões regulares sobre texto corrido, o que reduz a fragilidade da etapa. Aplica-se na sequência um função de correção de acentuação característicos de documentos produzidos por composição \LaTeX{} em português, frequentes no acervo nacional. Estas correções buscam uniformizar a representação de caracteres acentuados, e outros caracteres especiais da lingua portuguesa.
...
O objeto produzido não é arbitrário: ele decorre da própria fundamentação. \citeonline{GenresOrgComm} sustentam que gêneros são encenados por meio de \textbf{regras de gênero}, que associam elementos apropriados de forma e substância a situações recorrentes, e que tais regras operam com frequência de modo tácito, por socialização e hábito. O que o Fluxo A realiza, nestes termos, é a explicitação dessas regras a partir do único registro em que elas se encontram materialmente depositadas, que é o conjunto de textos que a comunidade reconheceu. Cada regra derivada é composta pelos elementos descritos na Tabela~\ref{tab:regra}.
...
\hline
Predicado & Condição verificável sobre a representação do manuscrito & A6 \\
\hline
...
O Modo Tutor executa duas formas de consulta, correspondentes a duas análises distintas sobre o trabalho do autor.

A \textbf{consulta à especificação} não pressupõe trilha-alvo e nem possui uma definição padrão(Default). O autor seleciona a trilha-alvo e recebe a especificação renderizada em linguagem natural com o resultado da análise e orientações para o autor: o que aquela trilha espera, com que frequência cada elemento ocorre entre os artigos aceitos, que função cada um cumpre e como artigos reais os realizaram. 
...
O manuscrito percorre o mesmo procedimento de A1 a A3, com o mesmo conversor, o mesmo extrator e o mesmo esquema de saída.

\subsection*{B3. Verificação de conformidade}

Cada regra da Especificação de Gênero tem seu predicado avaliado contra a representação do manuscrito. A operação é determinística e produz dois conjuntos: as regras \textbf{invocadas} pelo manuscrito e as regras \textbf{não invocadas}. Sobre o segundo conjunto aplica-se o filtro de estatuto, de modo que apenas as regras de estatuto esperado dão origem a observação, ao passo que a não realização de regras opcionais permanece sem consequência.

O resultado final da etapa não é uma pontuação. A recusa de emitir score agregado de conformidade é proposital, dado que uma nota converteria retorno formativo em julgamento e aproximaria o artefato do papel de revisor, além de atribuir uma responsabilidade que o artefato não tem. O propósito do Modo Tutor é orientar um pesquisador iniciante, não inferir sobre aceitação ou rejeição. 
...
\textbf{Dependência da qualidade da extração.} Toda a cadeia repousa sobre a acurácia do rotulador de A3. É esta a razão pela qual a validação técnica preliminar, visando medir a concordância entre os rótulos produzidos pelo modelo local e uma anotação manual de referência, em amostra reduzida, antes de qualquer processamento do corpus completo.
