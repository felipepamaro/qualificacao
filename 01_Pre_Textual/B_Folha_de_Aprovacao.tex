%------------------------------------------------------------------------------
% Início da folha de rosto
%------------------------------------------------------------------------------

\vspace{0.5cm}

\begin{center}
    INTELIGÊNCIA ARTIFICIAL NA ESCRITA CIENTÍFICA: \\
    UM TUTOR DE GÊNERO AUTOMATIZADO PARA SUBMISSÕES EM CONFERÊNCIAS BRASILEIRAS

    \vskip 0.5 cm

    Felipe Pereira Amaro
    \vskip 1.0 cm
\end{center}

\vspace{0.5cm}
\noindent

DISSERTATION SUBMITTED AS A PARTIAL REQUIREMENT FOR THE DEGREE OF MASTER IN INFORMATICS AT THE GRADUATE PROGRAM IN INFORMATICS OF THE FEDERAL UNIVERSITY OF THE STATE OF RIO DE JANEIRO (UNIRIO). APPROVED BY THE EXAMINING COMMITTEE BELOW.

\vskip 1.0 cm

\begin{flushleft}
    Approved By:
    \vskip 0.5 cm

\end{flushleft}

\begin{flushright}
    \rule{11.0cm}{.1mm} \\
    Ana Cristina Bicharra Garcia,~D.Sc. - UNIRIO
    \vskip 0.40 cm

    \rule{11.0cm}{.1mm}\\
    xxxx xxxxxxxxxxx xxxxx xxxxx,~D.Sc. - UNIRIO
    \vskip 0.40 cm

    \rule{11.0cm}{.1mm}\\
    xxxx xxxxxxxxxxx xxxxx xxxxx,~D.Sc. - UNIRIO
    \vskip 0.40 cm

    \rule{11.0cm}{.1mm}\\
    xxxxxxxxx xxxxxxxx xxxxxx,~D.Sc. - UFRJ
    \vskip 0.40 cm

    \rule{11.0cm}{.1mm}\\
    xxxxxxxx xxxxxx xxxxxxxx,~D.Sc. - USP
    \vskip 0.40 cm

    %\rule{11.0cm}{.1mm}\\
    %Dr.~ nome3 --- instituição
    %\vskip 0.75cm
    
%	\rule{10.0cm}{.1mm}
    
    \vskip 0.75cm
    
\end{flushright}
\vspace{\stretch{1}} % Posiciona o próximo texto no fim da página.
\begin{center}
    RIO DE JANEIRO, RJ - BRAZIL \\ OCTOBER 2025
\end{center}
\thispagestyle{empty}
\newpage
%------------------------------------------------------------------------------
% Fim da folha de rosto
%------------------------------------------------------------------------------
